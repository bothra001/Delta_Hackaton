...
@Composable
fun stulogin(navcontroller: NavController) {
    ...
    Column(
        ...
        Button(onClick = {
            coroutineScope.launch {
                try {
                    val response = RetrofitInstance.api.loginUser(UserLoginRequest(username, password))
                    if (response.isSuccessful) {
                        navcontroller.navigate("student home")
                    } else {
                        Log.e("UserLogin", "Error: ${response.errorBody()?.string()}")
                    }
                } catch (e: Exception) {
                    Log.e("UserLogin", "Exception: ${e.message}")
                }
            }
        }) {
            Text("Login")
